...
export class PolicyListComponent {
  ...
  showClaimButton: boolean = AppConfig.showClaimAddButton;
  ...
  advancedSearchConfig:any={
    inputConfig: [
      ...
      {isDisplay:true , propertyName:'policy_status', value:null, placeholder:'Select',label:'Policy Status', className:'form-control',type:'dropdown',inputType:'dropdown',
        data:[
          {key:'Active',value:1},{key:"30 Days Wait",value:2},{key:"Expired",value:3},{key:"Awaiting Escrow Payment",value:4},{key:"Do Not Charge",value:5},{key:"Hold (Failed Payment)",value:6},{key:"Pending (Link-Payment)",value:7}]//Pending (Link-Payment)
        },
  ...
  getPolicyList() {
    let api =`${AppConfig.apiUrl.policy.getAllPolicies}?page=${this.paginationObj.currentPage}&limit=${this.paginationObj.limit}&sortField=${this.sortField}&sortOrder=${this.sortOrder}${this.searchingvalue ? `&search=${this.searchingvalue}` : ''}${this.advanceFilter?this.advanceFilter:''}`
      ...
      next: (response: any) => {
        if (response.status==1) {
          this.advancedSearchConfig.inputConfig.forEach((element:any) => {
            if (element.propertyName=='policy_status') {
             element.data=[]
             response?.data?.all_policy_status.forEach((el:any) => {
 ...
 /*  search(e: any) {
  ...
  getSearchInputVal(str: string) {
    ...
    this.commonSvc.resetPagination(this.paginationObj,'/policy-management');
    this.getPolicyList();
  }
 ...
 async openViewPolicyModal(template: TemplateRef<any>, obj: any) {
  ...
  exportPolicyList(export_key:string){
    this.apiSvc.downloadFile(`${AppConfig.apiUrl.policy.getAllPolicies}/${export_key}?page=${this.paginationObj.currentPage}&limit=${this.paginationObj.limit}&sortField=${this.sortField}&sortOrder=${this.sortOrder}${this.searchingvalue?`&search=${this.searchingvalue}`:''}${this.advanceFilter?this.advanceFilter:''}`, '').subscribe({
      next: (response: any) => {
