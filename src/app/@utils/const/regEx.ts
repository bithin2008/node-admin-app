export const regEx = {
    ...
    strong_password: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[`~!@#$%^&*()_=\[\]{};':"\\|,.<>\/?+-])[A-Za-z\d`~!@#$%^&*()_=\[\]{};':"\\|,.<>\/?+-]{8,}$/,//^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{8,}$/,
    ...
    card_number_format: /^[0-9]{4}(?:\s[0-9]{4}){3}$/,
    card_name: /^[a-zA-Z\s\-'\.]{4,30}$/,
    card_expiry_date_format: /^(0[1-9]|1[0-2])\/\d{2}$/,
    aba_routing: /^\d{1,9}$/,
    ...
    onlyspace: /^\s*$/,
    fbPattern: /^(https?:\/\/)?(www\.)?facebook.com\/.*/i,
    instaPattern: /^(https?:\/\/)?(www\.)?instagram.com\/.*/i,
    youtubePattern: /^(https?:\/\/)?(www\.)?youtube.com\/.*/i,
    whatsappPattern: /^(https?:\/\/)?(www\.)?api\.whatsapp.com\/.*/i,
    twitterPattern: /^(https?:\/\/)?(www\.)?twitter.com\/.*/i,
    linkedinPattern: /^(https?:\/\/)?(www\.)?linkedin.com\/.*/i,
    pinterestPattern : /^https?:\/\/(?:www\.)?pinterest\.com\/[a-zA-Z0-9_-]+\/[a-zA-Z0-9_-]+\/?$/,
};